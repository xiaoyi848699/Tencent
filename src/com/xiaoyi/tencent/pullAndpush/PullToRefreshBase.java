...
public abstract class PullToRefreshBase<T extends View> extends LinearLayout {
	 ...
	 * 刷新完成
	 */
	protected void refreshCompleteResetHeader() {
		mState = PULL_TO_REFRESH;
		mIsBeingDragged = false;
		Log.d("TAG", "refreshCompleteResetHeader===================================================");
		if (null != mHeaderLayout) {
			mHeaderLayout.refreshCompleteReset();
		}
		if (null != mFooterLayout) {
			mFooterLayout.refreshCompleteReset();
		}
	...
	private void init(Context context, AttributeSet attrs) {
		...
		if (a.hasValue(PullToRefresh_mode)) {
			mMode = a.getInteger(PullToRefresh_mode, MODE_PULL_DOWN_TO_REFRESH);
		}
		...
		String completeLabel = context.getString(R.string.pull_to_refresh_complete);
		...
		if (mMode == MODE_PULL_UP_TO_REFRESH || mMode == MODE_BOTH) {
            mFooterLayout = new LoadingLayout(context, MODE_PULL_UP_TO_REFRESH, releaseLabel, pullLabel, refreshingLabel,completeLabel);
		...
		if (a.hasValue(PullToRefresh_headerBackground)) {
			setBackgroundResource(a.getResourceId(PullToRefresh_headerBackground, Color.WHITE));
		}
		if (a.hasValue(PullToRefresh_adapterViewBackground)) {
			mRefreshableView.setBackgroundResource(a.getResourceId(PullToRefresh_adapterViewBackground,
					Color.WHITE));
		}
		a.recycle();
